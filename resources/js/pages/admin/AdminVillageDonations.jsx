...
export default function AdminVillageDonations() {
  const [donations, setDonations] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    adminService.listAllDonations({ limit: 200, targetType: 'village' })
  ...
  return (
    <div className="min-h-screen bg-background">
      <HeroScrollSection size="compact">
        <AdminPageHeader title="Village Donations" subtitle={`₹${total.toLocaleString('en-IN')} received across villages`} />
      </HeroScrollSection>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-border overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-20 text-muted-foreground"><Heart className="w-12 h-12 mx-auto mb-3 opacity-30" />No village donations yet</div>
          ) : (
              ...
              <tbody className="divide-y">
                {donations.map((d) => (
                  <tr key={d.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-3">{d.villages?.village_name || '—'}</td>
