...
export default function AdminSchoolActivities() {
  const [activities, setActivities] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    adminService.listActivityLogs({ loggableType: 'school' })
      .then(setActivities)
      .catch(() => setActivities([]))
  ...
  return (
    <div className="min-h-screen bg-background">
      <HeroScrollSection size="compact">
        <AdminPageHeader title="School Activities" subtitle="Timelines from `activity_logs` for schools" gradient="school-gradient" />
      </HeroScrollSection>
        ...
        <div className="bg-white rounded-2xl border border-border overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              ...
              No school activities logged yet.
            </div>
          ) : (
            <table className="w-full text-sm min-w-[640px]">
              <thead><tr className="border-b bg-muted/30">
                ...
                <th className="text-left px-4 py-3">School ID</th>
              </tr></thead>
